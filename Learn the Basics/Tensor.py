...
张量与[NumPy](https://numpy.org/)的ndarrays类似，只是张量可以在GPU或其他硬件加速器上运行。事实上，张量和NumPy数组通常可以
共享相同的底层内存，不需要复制数据（见[与NumPy的桥接]
(https://pytorch.org/tutorials/beginner/blitz/tensor_tutorial.html#bridge-to-np-label)）。张量还为自动求导进行
了优化（我们将在后面的[Autograd](https://pytorch.org/tutorials/beginner/basics/autogradqs_tutorial.html)部分看到
更多关于这一点）。如果你熟悉ndarrays，你就会对张量API的很熟悉。如果不熟悉，请跟上!
...
# ### 从NumPy数组中创建
#
# 张量可以用NumPy数组创建（反之亦然，详见与[NumPy桥接]
...
# 默认情况下，张量是在CPU上创建的。我们需要使用 `.to` 方法显式地将张量移动到GPU上（在检查GPU的可用性之后）。请注意，在不同的设备上复制大型的张量，
# 在时间和内存上都是很昂贵的。

if torch.cuda.is_available(): # 如果GPU可用，我们就将张量移动到GPU上。
...
###################################################
# **拼接张量** 你可以使用`torch.cat` 来沿着给定的维度拼接一组张量。也请参见
# [torch.stack](https://pytorch.org/docs/stable/generated/torch.stack.html)，另一个与`torch.cat`有细微差别的张量连接运算符。
